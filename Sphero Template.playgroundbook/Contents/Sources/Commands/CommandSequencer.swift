...
public enum ToyDeviceResponseCode: UInt8 {
    case didSleep = 0x14
}

public protocol CommandResponse {}

public protocol AsyncCommandResponse: CommandResponse {}
public protocol DeviceCommandResponse: CommandResponse {}

private protocol ResponseParser {
    func parseData(_ data: Data, responseId: UInt8, deviceId: CommandDeviceId) -> CommandResponse?
}

private struct AsyncCommandResponseParser: ResponseParser {
    func parseData(_ data: Data, responseId: UInt8, deviceId: CommandDeviceId = .spheroCommandDeviceId) -> CommandResponse? {
        ...
        if let responseCode = ToyAsyncResponseCode(rawValue: responseId) {
            ...
            case .sensorDataStreaming:
                return SensorData(data: choppedData)
            case .collisionDetected:
                return CollisionData(data: Array(choppedData))
...
public final class CommandSequencer {
    ...
    typealias ParserCallback = ((_ sequencer: CommandSequencer, _ response: CommandResponse?) -> Void)?
    ...
    fileprivate var commandSequence: [UInt8 : Command] = [:]
...
extension CommandSequencer {
    public func data(from command: Command) -> Data {
