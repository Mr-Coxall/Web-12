...
fileprivate func valueFrom(data: Data, startLocation: Int) -> Int {
    ...
    let bigByte = UInt16(dataArray[startLocation]) << 8
    let littleByte = UInt16(dataArray[startLocation + 1])
    
    let total = bigByte + littleByte
    let value = UInt(total)
    let intValue = Int16(truncatingBitPattern: value)
    return Int(intValue)
}

public struct SensorDataCommandResponse: AsyncCommandResponse {
    ...
    public init(data: Data) {
        var location = 0
        let sensorMask = DataStreamingDataTracker.sensorMask!
        ...
        if !sensorMask.isDisjoint(with: [.gyroXRaw, .gyroYRaw, .gyroZRaw]) {
            ...
            if sensorMask.contains(.gyroXRaw) {
                rawGyro.x = valueFrom(data: data, startLocation: location)
                location = location + 2
            }
            if sensorMask.contains(.gyroYRaw) {
                rawGyro.y = valueFrom(data: data, startLocation: location)
                location = location + 2
            }
...
/// The streaming sensor data.
public typealias SensorData = SensorDataCommandResponse

struct DataStreamingDataTracker {
    static var frameCount = 0
    static var sensorMask: SensorMask?
}
