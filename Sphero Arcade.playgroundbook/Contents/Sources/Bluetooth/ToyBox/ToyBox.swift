...
public final class ToyBox {
    ...
    public lazy var centralManager: PlaygroundBluetoothCentralManager = { [weak self] in
        let centralManager = PlaygroundBluetoothCentralManager(services: [.robotControlService], queue: .main)
    ...
    func putAway(toy: Toy) {
        toy.core.send(GoToSleepCommand(type: .sleep))
    }
    
    func disconnect(toy: Toy) {
        guard let toy = connectedToys[toy.identifier] else { return }
        centralManager.disconnect(from: toy.peripheral)
    }
    ...
    func centralManager(_ central: PlaygroundBluetoothCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String: Any]?, rssi: Double) {
        ...
        guard let advertisedPower = advertisementData[CBAdvertisementDataTxPowerLevelKey] as? NSNumber else { return }
    ...
    func centralManager(_ central: PlaygroundBluetoothCentralManager, didConnect peripheral: CBPeripheral) {
        let toyCore = Toy.ToyCore(peripheral: peripheral)
        toyCore.prepareConnection { didPrepareConnection, error in
            if didPrepareConnection {
                ...
                case let sprk where (sprk.hasPrefix(SPRKToy.descriptor)):
                    toy = SPRKToy(identifier: peripheral.identifier, core: toyCore, owner: self)
                case let bb8 where (bb8.hasPrefix(BB8Toy.descriptor)):
                    toy = BB8Toy(identifier: peripheral.identifier, core: toyCore, owner: self)
                default:
                    break
                }
